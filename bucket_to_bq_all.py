...
def get_table_load_configs() -> list[dict[str, str]]:
    table_configs: list[dict[str, str]] = []
    excluded_prefixes = {"TABLE"}

    for env_name, env_value in os.environ.items():
        if not env_name.endswith("_GCS_URI"):
            continue

        prefix = env_name[: -len("_GCS_URI")]

        if prefix in excluded_prefixes:
            continue

        table_id_env_name = f"{prefix}_TABLE_ID"
        table_id = os.getenv(table_id_env_name)

        if not table_id:
            raise ValueError(
                f"Missing required environment variable: {table_id_env_name}"
            )
    ...
    if not table_configs:
        raise ValueError("No *_TABLE_ID and *_GCS_URI mappings were found in .env.")
...
def build_select_expression(
    ...
    if field.field_type == "STRING":
        return f"UPPER(CAST({source_column_reference} AS STRING)) AS `{field.name}`"
...
def load_all_tables() -> None:
    ...
    project_id = shared_config["project_id"]
    dataset_id = shared_config["dataset_id"]
    table_configs = get_table_load_configs()
    ...
    if failed_tables:
        raise RuntimeError(
            "One or more table loads failed: " + ", ".join(failed_tables)
        )
